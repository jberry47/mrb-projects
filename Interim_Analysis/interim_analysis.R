...
#-- USING HOURS AS OBSERVED VALUES
...
#-- USING DAYS AS OBSERVED VALUES
#-- Specify priors to use in interim analysis
my_priors <- list(m = c(mean(dat$days_2nd_normal[dat$treatment_group == "Y"]), mean(dat$days_2nd_normal[dat$treatment_group == "Z"])), n = c(1, 1), s2 = c(var(dat$days_2nd_normal[dat$treatment_group == "Y"]), var(dat$days_2nd_normal[dat$treatment_group == "Z"])))

#-- Interim analysis with the data at hand
res <- bayes_normal(
  sample1 = dat$days_2nd_normal[dat$treatment_group == "Y"],
  sample2 = dat$days_2nd_normal[dat$treatment_group == "Z"],
  priors = my_priors,
  rope_int = c(-1,1),
  plot = T
)
...
#-- Conclusions
# Using the currently observed data to estimate the distibutions of the two treatments as priors, we draw samples from them to increase the total replicates per arm to 27, and this is done 2,000 times. In each iteration of the simulation, we estimate the percent chance that the difference between treatments is with a 1day window. We summarize each iteration's percent chance using arithmatic mean to show that the overall percent chance of a non-clinically actionable outcome is 95.4%, in otherwords the probability of a clinically actionable outcome is only 5%. 
